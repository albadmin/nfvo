...
public class NetworkServiceRecordManagement
  ...
  public VirtualNetworkFunctionRecord restartVnfr(
    ...
    // set nsr status to null since the virtual resources (virtual machines)
    // are going
    // to be rebuilt. Setting the status not to ACTIVE also prevent other
  ...
  private NetworkServiceRecord scaleOutNsr(
    ...
    int attempt = 0;
    // this while loop is necessary, because while creating the NSR also a
    // VIM might be changed
    ...
    while (!savedNsrSuccessfully) {
      ...
      for (VirtualDeploymentUnit vdu : vnfd.getVdu()) {
        ...
        // if no VIMs were provided, then use all available VIMs in the
        // used project
        ...
        // vduVimInstances now contains all possible vim instance names
        // for a specific vdu
      ...
      // TODO it better: Check if the chosen VIM has ENOUGH Resources for
      // deployment
      // checkQuotaForNS();
    ...
    // If the new VNF is a target, we need to fill the dependencies of its
    // sources
  ...
  public void deleteVNFRecord(String idNsr, String idVnf, String projectId)
    ...
    // TODO the logic of this request for the moment deletes only the VNFR
    // from the DB, need to be
    // removed from the
    ...
    for (VirtualNetworkFunctionRecord vnfr : networkServiceRecord.getVnfr()) {
      ...
      for (VirtualDeploymentUnit vdu : vnfr.getVdu()) {
        for (VNFCInstance instanceInVNFR : vdu.getVnfc_instance()) {
          ...
          // if vnfciStarted is not null then the START message
          // received refers to the VNFCInstance
  ...
  private NetworkServiceRecord deployNSR(
    ...
    log.debug("Check was successfull: all vnfms are registered and active");
    ...
    while (!savedNsrSuccessfully) {
      ...
      for (VirtualNetworkFunctionDescriptor virtualNetworkFunctionDescriptor :
        ...
        for (VirtualDeploymentUnit vdu : virtualNetworkFunctionDescriptor.getVdu()) {
          ...
          vduVimInstances.put(vdu.getId(), instanceNames);
          // vduVimInstances now contains all possible vim instance
          // names for a specific vdu
          for (String vimInstanceName : instanceNames) {
          ...
          checkIfVimAreSupportedByPackage(virtualNetworkFunctionDescriptor, instanceNames);
          // if no VIMs were provided, then use all available VIMs in
          // the used project
          if (instanceNames.isEmpty()) {
            log.info(
                "No VIM instances specified in VDU "
      ...
      try {
        ...
        log.info("OptimisticLockingFailureException while creating the NSR. We will try it again.");
        savedNsrSuccessfully = false;
        attempt++;
      }
    }
  ...
  private void checkSshInfo(NetworkServiceDescriptor nsd, DeployNSRBody body)
    ...
    for (VirtualNetworkFunctionDescriptor vnfd : nsd.getVnfd()) {
      ...
      // - no username and no password is allowed because this
  ...
  private void fillDeploymentTimeIPs(
    ...
    for (VirtualNetworkFunctionDescriptor virtualNetworkFunctionDescriptor :
      ...
      // Here we assume the VNFD contains only one VDU and one VNF
      // component.
            ...
            for (String vimInstanceName : vdu.getVimInstanceName()) {
              ...
              if (vimInstance != null && vimInstance instanceof OpenstackVimInstance) {
                ...
                for (DeploymentFlavour deploymentFlavour : osVimInstance.getFlavours()) {
                  // TODO: Should find a better solution for
                  // here and generic
  ...
  private Set<String> checkIfVimAreSupportedByPackage(
    ...
    log.info("Performing VIM type check for VIM instances...");
    ...
    for (String vimInstanceName : instanceNames) {
      for (BaseVimInstance vimInstance :
        ...
        if (getVimNameWithoutAvailabilityZone(vimInstanceName).equals(vimInstance.getName())) {
          log.info("Found vim instance " + vimInstance.getName());
          log.info(
              "Checking if "
                  + vimInstance.getType()
                  + " is contained in "
    ...
    log.info("Vim Instances chosen are: " + checkedVimInstanceNames);
  ...
  public VirtualNetworkFunctionRecord upgradeVnfr(
    ...
    // Use the first image found in the VDUs TODO: improve image choice for
    // upgrade/rebuild
    ...
    // Rebuild the VNF using the upgraded VNF Descriptor and VNF Package and
    // after the rebuild will
    // be finished the
  ...
  public void executeAction(
    ...
    switch (nfvMessage.getAction()) {
      case HEAL:
        // Note: when we get a HEAL message from the API, it contains only
        // the cause (no vnfr or
        // vnfcInstance).
        // Here the vnfr and the vnfcInstance are set into the message,
        // since they are updated.
  ...
  public void delete(String id, String projectId)
    ...
    if (!networkServiceRecord.getVnfr().isEmpty()) {
      networkServiceRecord.setStatus(Status.TERMINATED); // TODO maybe
      // terminating?
      for (VirtualNetworkFunctionRecord vnfr : networkServiceRecord.getVnfr()) {
  ...
  public void resume(String id, String projectId)
    ...
    for (VNFRecordDependency vnfrDependency : networkServiceRecord.getVnf_dependency()) {
      // Check for sources and target ready to have their dependencies
      // resolved
      ...
      if (vnfrTarget.getStatus().ordinal() == (Status.INITIALIZED.ordinal())) {
        ...
        if (readyToResolve) {
          ...
          // Retrieve from VNFR Dependency Repository the dependency
          // record for VNFR target with
          // ready dependencies
