...
public abstract class AbstractTask implements org.openbaton.vnfm.interfaces.tasks.AbstractTask {
  ...
  public NFVMessage call() {
    ...
    if (result == null) {
      ...
      log.debug(
          "--- AbstractTask: Publishing eventNFVO for VNFR: "
  ...
  private NFVMessage handleVimException(VimException e) {
    ...
    if (vnfcInstance != null) {
      log.info(
          "--- AbstractTask: The VM was not correctly deployed. ExtId is: "
              + vnfcInstance.getVc_id());
      log.debug("--- AbstractTask: Details are: " + vnfcInstance);
      for (VirtualDeploymentUnit vdu : virtualNetworkFunctionRecord.getVdu()) {
        if (vdu.getId().equals(e.getVirtualDeploymentUnit().getId())) {
          ...
          log.debug("--- AbstractTask: Found VDU and set vnfcInstance");
        }
      }
    }
  ...
  private NFVMessage handleVimDriverException(VimDriverException e) {
    e.printStackTrace();
    log.error("--- AbstractTask:" + e.getMessage());
    HistoryLifecycleEvent lifecycleEvent = new HistoryLifecycleEvent();
    lifecycleEvent.setEvent(Event.ERROR.name());
    lifecycleEvent.setDescription(e.getMessage());
    SimpleDateFormat format = new SimpleDateFormat("yyyy.MM.dd'-'HH:mm:ss:SSS'-'z");
  ...
  private void genericExceptionHandling(Exception e) {
    e.printStackTrace();
    log.debug("--- AbstractTask: The exception is: " + e.getClass().getName());

    if (e.getCause() != null) {
      log.debug("--- AbstractTask: The cause is: " + e.getCause().getClass().getName());
    }
  ...
  private void changeStatus() {
    log.debug("--- AbstractTask: Action is: " + action);
    Status status = virtualNetworkFunctionRecord.getStatus();
    log.debug("--- AbstractTask: Previous status is: " + status);
    switch (action) {
      ...
      case START:
        {
          ...
          for (VirtualDeploymentUnit vdu : virtualNetworkFunctionRecord.getVdu()) {
            for (VNFCInstance instanceInVNFR : vdu.getVnfc_instance()) {
              ...
              log.debug("--- AbstractTask: VNFCInstance: " + instanceInVNFR.getHostname());
    ...
    log.debug(
        "--- AbstractTask: Changing status of VNFR: "
            + virtualNetworkFunctionRecord.getName()
            + " ( "
            + virtualNetworkFunctionRecord.getId()
            + " ) to "
  ...
  protected VirtualNetworkFunctionRecord getNextToCallStart(
    ...
    if (vnfrNames != null) {
      ...
      log.debug("--- AbstractTask: List of VNFRs to start: " + vnfrNames);
