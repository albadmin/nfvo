...
public class InstantiateTask extends AbstractTask {
  ...
  protected NFVMessage doWork() throws Exception {
    ...
    log.debug(
        "--- InstantiateTask: Filled dependency parameters of "
            + virtualNetworkFunctionRecord.getName());
    saveVirtualNetworkFunctionRecord();
    log.debug("--- InstantiateTask: Saved VNFR " + virtualNetworkFunctionRecord.getName());
    NetworkServiceRecord nsr =
        networkServiceRecordRepository.findFirstById(
            virtualNetworkFunctionRecord.getParent_ns_id());
    log.debug("--- InstantiateTask: The current status of the NSR is " + nsr.getStatus());
    for (VirtualNetworkFunctionRecord vnfr : nsr.getVnfr()) {
      log.trace("--- InstantiateTask: Current VNFRs in the database: " + vnfr.getName());
    }
    dependencyQueuer.releaseVNFR(virtualNetworkFunctionRecord.getName(), nsr);
    log.info(
        "--- InstantiateTask: Calling dependency management for VNFR: "
            + virtualNetworkFunctionRecord.getName());
    int dep = dependencyManagement.provisionDependencies(virtualNetworkFunctionRecord);
    log.info("--- InstantiateTask: Found " + dep + " dependencies");
    log.trace(
        "--- InstantiateTask: Is ordered execution of VNFs enabled? "
            + Boolean.parseBoolean(ordered));
  ...
  private void sendStart(VirtualNetworkFunctionRecord virtualNetworkFunctionRecord)
    ...
    log.info(
        "--- InstantiateTask: Calling START to: "
            + virtualNetworkFunctionRecord.getName()
            + " because it has 0 dependencies");
    log.trace(
        "--- InstantiateTask: VNFR ("
            + virtualNetworkFunctionRecord.getId()
